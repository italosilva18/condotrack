...
func (h *PortalHandler) ProxyGeminiAI(c *gin.Context) {
	...
	if h.cfg.GeminiAPIKey == "" {
		response.InternalError(c, "Gemini API key not configured")
		return
	}
	...
	// Call Gemini API
	url := fmt.Sprintf("https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=%s", h.cfg.GeminiAPIKey)

	httpReq, err := http.NewRequestWithContext(c.Request.Context(), "POST", url, bytes.NewBuffer(jsonData))
	if err != nil {
	...
	if err != nil {
		response.InternalError(c, "Failed to call Gemini API: "+err.Error())
		return
	}
	...
	if resp.StatusCode != http.StatusOK {
		response.InternalError(c, fmt.Sprintf("Gemini API error: %s", string(body)))
		return
	}
...
func (h *PortalHandler) UploadEvidence(c *gin.Context) {
	...
	if err != nil {
		response.InternalError(c, "Failed to upload evidence: "+err.Error())
		return
	}
...
func (h *PortalHandler) ListEvidence(c *gin.Context) {
	...
	if err != nil {
		response.InternalError(c, "Failed to list evidence files: "+err.Error())
		return
	}
...
func (h *PortalHandler) DeleteEvidence(c *gin.Context) {
	...
	if err != nil {
		response.InternalError(c, "Failed to delete evidence: "+err.Error())
		return
	}
