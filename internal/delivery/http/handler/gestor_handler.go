...
func (h *GestorHandler) ListGestores(c *gin.Context) {
	...
	if includeContracts {
		...
		if err != nil {
			response.InternalError(c, "Failed to fetch gestores: "+err.Error())
			return
		}
...
func (h *GestorHandler) GetGestorByID(c *gin.Context) {
	...
	if err != nil {
		response.InternalError(c, "Failed to fetch gestor: "+err.Error())
		return
	}
...
func (h *GestorHandler) CreateGestor(c *gin.Context) {
	...
	if err != nil {
		...
		response.InternalError(c, "Failed to create gestor: "+err.Error())
		return
	}
...
func (h *GestorHandler) UpdateGestor(c *gin.Context) {
	...
	if err != nil {
		...
		response.InternalError(c, "Failed to update gestor: "+err.Error())
		return
	}
...
func (h *GestorHandler) DeleteGestor(c *gin.Context) {
	...
	if err != nil {
		...
		response.InternalError(c, "Failed to delete gestor: "+err.Error())
		return
	}
