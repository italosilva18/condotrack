...
func (h *CertificadoHandler) GetCertificatesByStudent(c *gin.Context) {
	...
	if err != nil {
		response.InternalError(c, "Failed to fetch certificates: "+err.Error())
		return
	}
...
func (h *CertificadoHandler) GetCertificateByID(c *gin.Context) {
	...
	if err != nil {
		response.InternalError(c, "Failed to fetch certificate: "+err.Error())
		return
	}
...
func (h *CertificadoHandler) ValidateCertificate(c *gin.Context) {
	...
	if err != nil {
		response.InternalError(c, "Failed to validate certificate: "+err.Error())
		return
	}
